const fs = require('fs');
const p = 'C:/Users/BaoGo/.cursor/projects/c-Users-BaoGo-Documents-ClaudeCode-Project/agent-transcripts/f2384e48-d3e2-4864-a12b-9a5b9e520f5a/f2384e48-d3e2-4864-a12b-9a5b9e520f5a.jsonl';
const outCR = 'c:/Users/BaoGo/Documents/ClaudeCode_Project/Learn_How_To_Love/Demo/js/choice-reactions.js';
  ...
  try {
    ...
    for (const c of o.message?.content || []) {
      if (c.name === 'Write' && c.input?.path?.includes('choice-reactions.js')) {
        ...
        if (contents.includes('CHOICE_REACTIONS') && !contents.startsWith('const fs = require')) {
          cr = contents;
        }
      }
      if (c.name === 'StrReplace' && c.input?.path?.includes('choice-reactions.js')) {
        crPatches.push(c.input);
      }
    }
  } catch (_) {}
}
...
if (!cr) {
  console.error('No Write found for choice-reactions');
  process.exit(1);
}
...
// dogLabel + naming updates
cr = cr.replace(/\$\{s\.dogName\}/g, '${dogLabel(s)}');
cr = cr.replace(
  "  'prologue_home::Day 2 的早晨……': {\n    text: (s) => `${dogLabel(s)} 在紙箱邊睡著了，呼吸比昨夜穩一點。`,\n    feeling: 'sleepy', cue: 'sigh', holdMs: 1100,\n  },\n\n  'day2_morning::",
...
fs.writeFileSync(outCR, cr, 'utf8');
console.log('choice-reactions restored, patches applied:', applied);
