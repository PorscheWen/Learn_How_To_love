...
4. **中年篇深度**：寫 routine、被忽略、家庭變化——Steam 上幾乎無人做滿。
...
**主題：** 日子變普通了，愛藏在 routine 裡。**（系列最大差異化章節）**
...
| 感受 | 常見觸發 | 狗的行為 | 玩家回應 |
|------|----------|----------|----------|
| **Anxious 不安** | 独处、陌生、被吼 | 踱步、低吼、不進食 | 陪伴、安撫、固定 routine |
...
| **Sleepy 困倦** | 午後、安靜 routine | 眼皮半闔、打盹 | 不打扰、维持节奏 |
| **Playful 玩耍** | 玩具、互动成功 | 前脚趴低、摇尾 | 一起参与、适度引导 |
| **Alert 警戒** | 雷声、陌生声响 | 竖耳、身体绷紧 | 陪伴、不强迫 |
| **Shy 害羞** | 新环境、刚靠近 | 藏脸、只露鼻尖 | 等待、让牠决定距离 |
| **Hungry 饥饿** | 喂食、厨房气味 | 鼻子贴地、循味 | 固定 routine、不催促 |
| **Angry 生气** | 被吼、被关 | 低吼、耳朵后贴 | 修复、不硬拉 |
...
| `dog-kitchen.png` | `kitchen` | 碗邊嗅聞 | 廚房、餵食 routine |
...
- 新章節優先新增**故事動作圖**（Memory / Landmark 專用 pose），情緒圖可复用。
- 背景場景用 CSS／背景圖；**狗角色永遠是独立透明 PNG 叠在场景上**，不画进背景里。
...
*最後更新：2026-06-26（新增 §6.6 水彩 PNG 狗角色資產規範）。*
