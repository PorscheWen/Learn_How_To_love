...
4. 玄關：胸背帶＋牽繩（×0.8）；高信任貼腳、低信任停門線／鞋櫃邊。
...
| 玄關 | `char-yuan-leash` | `dog-leash-wait` | 沿用 S08 牽繩姿；transform **×0.8** |
...
| 場 | 朝向 | Transform（摘要） |
|----|------|-------------------|
| 客廳告別 | 圖檔予安面**左**、狗抬頭偏左；畫面予安左／狗右 → **`xzoom -0.36` 翻予安**後對望 | `char_right_farewell` 0.40／×0.8；`dog_farewell_near` 0.62／0.272 |
| 玄關 | 予安蹲姿→左；狗 leash-wait→右 | `char_right_s09` zoom **0.36**；`dog_entrance_*_s09` ×0.8（far 0.208／mid 0.272） |
| 咖啡廳人物 | 同事蹲左→右；予安站右→左 | `char_*_cafe` zoom **0.36**（×0.8） |
| 拒絕 | 狗貼腳、面向同事 | `dog_cafe_near_guard`（`xzoom -0.272`／`yzoom 0.272`） |
| 僵住／交繩 | 狗在中間、面向予安 | `dog_cafe_mid` zoom 0.24 |
| 留下 | 狗貼鞋、面向予安 | `dog_cafe_near_home` zoom 0.272 |

**原則：** 人／狗朝向以當下劇情＋**實際圖檔面相**為準；翻轉時 `xzoom`／`yzoom` 同絕對值（勿 `zoom` 後再 `xzoom -1` 拉寬）。S09 全段立繪統一 **×0.8**。
...
*更新：2026-07-28｜客廳告別圖、玄關 ×0.8、咖啡廳專用立繪與劇情朝向*
