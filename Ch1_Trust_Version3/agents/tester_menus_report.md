...
> 日期：2026-07-29（設定空白修正：2026-07-30；輔助區擠返回：2026-07-31）  
...
**靜態：設定頁契約通過（無 yalign 推位、輔助 2×2 網格）。**  
已知修復：
1. 設定頁曾因 `viewport`＋`yfill` 嵌在 `game_menu` 的 `side` 中央而**高度歸零、整頁空白** → 已改回 `vbox`。
2. 加「輔助需求」後直排過高、擠到「返回」→ 改 `grid 2 2` 壓高度；大字體不再撐高 embed 鈕（設定／返回）。
