...
init python:
    ...
    def play_bgm(profile, fade=2.0):
        ...
        # 章節直達會先 stop；若下一首檔名與淡出中的曲相同，if_changed 會略過重播→靜音。
        # 因此：無聲、換檔、或 profile 已清空時，一律強制 play。
        need_play = (
            not playing
            or playing != filename
            or store._current_bgm is None
            or store._current_bgm != profile
        )
...
# S03 梯廳門外狗窩：靠公寓門（左側門墊／牆角；與 day／night 同構圖），勿落在中央樓梯口；狗 ×0.9（0.26×0.9）
...
# S04 尾隨：從椅旁地板走向廚房方向（畫面左側門線），搭配 dog wag 動畫。
...
label section_01_fluorescent_over_moon:
    ...
    "公寓門「嘀」一聲。冰箱低沉的嗡嗡，像廚房裡有人在輕輕唸經——其實只是壓縮機上班。"
    "她坐在沙發邊緣吃便當。電視沒開：黑螢幕比較省事，也比較不會質問她一天過得怎樣。"

    "吃到一半，手機亮起。備忘錄搜尋建議浮出三個月前的舊項目：「週六十點，動保處——帶租約影本。」"
...
label section_02_backdoor_glance:
    ...
    "脖子附近的毛也沒有被項圈壓平過的痕跡——沒有前任主人的縮寫，也沒有方便推託的故事。"
    ...
    menu:
        "蹲下，側身等待（不伸手）":
            ...
            with Dissolve(1.0)
            $ dog_sfx("soft")
            "狗靠近。停。再靠近。舌尖點了一下米粒，又點一下。每吞一口就抬眼看她——信任是分期付款，每期都要對帳。"
            "她的手指動了一下，又收回袖口。"
            thought "我可以什麼都不做。"
    ...
    if flags["dist_ok"]:
        show dog halfstep at dog_mid
        with Dissolve(0.6)
        "狗往前半步，停在「既不碰你、也不放你走」的距離。尾巴不搖，輕貼腿側。中立，但不是冷漠。"
        pause 0.6
    ...
    ya "今晚不算數。"
    "聲音輕得像怕被巷子聽見。巷子通常什麼都聽見，只是不急著回應。"

    "她把空便當盒收進袋子，留一點水在瓶蓋裡推過去。"
    if flags["dist_ok"]:
        show dog halfstep at dog_mid
        with Dissolve(0.5)
        "狗嗅了嗅，喝了兩口。水聲很小，舌尖一下一下點著瓶蓋邊緣。很小的聲音，很大的進度。"
    elif flags.get("s02_conscience_return", False):
        show dog halfstep at dog_mid
        with Dissolve(0.5)
...
label section_03_gate_temp_border:
    ...
    if flags.get("vet_first", False):
        "從夜間急診回來時，已經過了午夜。醫生的結論很務實：餓、脫水，外加一身要慢慢養的疲倦。沒有戲劇性的病名——代表接下來全是日常。"
...
label section_06_corridor_third_person:
    ...
    "[dog_label]的鼻子急促動了兩下：膠帶、陌生洗衣精，還有一隻停在頭頂上方的手。資訊量對一隻狗來說有點多。"
...
label section_10_share_the_key:
    ...
    if flags.get("gave_away", False):
        ...
        "回到客廳，[dog_label]先聞了聞紙袋，再退回熟悉的距離，看她把牆面擦乾，一臉監工的樣子。"
