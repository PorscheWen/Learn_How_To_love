...
default persistent.seen_howto = False
default persistent.viewed_secret_content = []
...
init python:
    ...
    CHAR_REF_H = 1280.0   # 歸一化畫布高；畫面 zoom 見 scale.rpy SCALE／SCALE_S02
    DOG_REF_H = 1536.0    # 歸一化畫布高；畫面 zoom 見 scale.rpy；可見高另乘 DOG_POSE_SCALE
    ...
    # sniff-wire 例外：僅 S05 使用，對齊客廳 parallel 可見高。
    ANIM_POSE_SCALE = {
        "door-sleep": 0.42,
        "back-sleep": 0.427,
        "check-sleep": 0.452,
...
init python:
    ...
    # ========== 彩蛋名字判定 ==========
    # 先對常見狗名（精確）；其餘含疊字（毛毛、樂樂、Momo）走通用 key。
    # 預設超商名「小7／小七」不觸發，避免沒改名也跳出彩蛋。
    COMMON_DOG_NAME_KEYS = {
        "小黑": "color_black",
        "黑黑": "color_black",
        "阿黑": "color_black",
        "小白": "color_white",
    ...
    EASTER_EGG_REACTIONS = {
        "color_black": "蜂蜜褐的毛被叫成這個名字。牠歪頭看她，停了半秒。",
        "color_white": "胸口那塊奶油白被點名。其餘的毛卻不是白的。牠低頭聞自己。",
        "color_yellow": "蜂蜜褐被叫成這個名字，還算接近。耳朵轉過來，又放下。",
        "color_spotted": "毛色並不花。牠還是把鼻子往前伸了一點，像在核對。",
        "fortune": "這名字聽起來很旺。牠不懂，只把尾巴尖搖了一下。",
        "food": "這名字像某種味道。鼻尖動了兩下，沒有找到對應的食物。",
        "woof": "牠短短哼了一聲。不是兇，比較像被這個音嚇了一跳。",
        "cat_sound": "這個音落到玄關，耳朵動了兩下，沒有走近。",
        "fluffy": "毛並不蓬。聽見這個名字，牠還是把耳朵轉過來，像在核對是不是在叫自己。",
        "round": "瘦長的身體配不上這個名字。尾巴尖還是搖了一下。",
        "spot": "胸口那塊奶油白被點名。牠低頭聞自己，又抬頭看她。",
        "itchy": "鼻尖往鞋櫃蹭了一下，像這個名字有點癢。",
        "moon": "走廊燈管很白。牠聽見名字，抬頭看了一眼天花板的光。",
        "ogau": "這兩個字很像在叫「狗」。牠的耳朵豎了一下，又貼回去。",
        "baby": "這名字軟軟的。耳朵轉過來，身體卻還停在原處。",
        "title": "這名字比牠此刻的樣子大一號。尾巴尖還是搖了一下。",
        "english": "陌生的音節從她嘴裡出來。耳朵轉過去，沒有走近。",
        "redup": "兩個一樣的音連在一起。耳朵轉了兩下，像在對拍。",
    }

    def _normalize_pet_name(name):
        return str(name or "").replace("\u3000", " ").strip()

    def _has_cjk_reduplication(name):
        """含連續重複的漢字：毛毛、樂樂、小毛毛、阿旺旺。"""
    ...
    def _has_latin_reduplication(name):
        """拉丁疊音：Momo、Lulu、Coco、Kiki。"""
        s = name.strip()
        if not s or not s.isalpha() or not s.isascii():
            return False
        n = len(s)
        if n % 2 or n < 2 or n > 6:
            return False
        half = n // 2
        return s[:half].casefold() == s[half:].casefold()

    def check_easter_egg_name(name):
        """
        檢查取名是否觸發彩蛋。
        回傳彩蛋 key（用於 S06 隱藏台詞），或 None（無彩蛋）。
        寫入：S05 取名後 flags["easter_egg_name"]
        讀取：S06 玄關隱藏一句
        """
        normalized = _normalize_pet_name(name)
        if not normalized or normalized in ("小7", "小七"):
            return None
        folded = normalized.casefold()
        if folded in COMMON_DOG_NAME_KEYS:
            return COMMON_DOG_NAME_KEYS[folded]
        if _has_cjk_reduplication(normalized) or _has_latin_reduplication(normalized):
            return "redup"
        return None

    def easter_egg_reaction_line():
        """S06 取名彩蛋一句；無 key 則不顯示。"""
        key = (store.flags or {}).get("easter_egg_name")
        if not key:
            return None
        return EASTER_EGG_REACTIONS.get(key)

    def s10_footprint_line(gave_away=False):
        """S10 依本周目 flag 組一句足跡；無旗標則不顯示。"""
        flags = store.flags or {}
        bits = []
        if flags.get("s02_looked_again"):
            bits.append("後門那一次對視")
        if flags.get("s03_returned"):
            bits.append("鐵門那個角度")
        elif flags.get("s03_choice") == "shoo":
            bits.append("牆角那一團")
        if flags.get("s04_parallel") or flags.get("ch2_seed_water_bowl"):
            bits.append("廚房那條線")
        if flags.get("s06_protected"):
            bits.append("樓梯間那半步")
        if flags.get("s07_fetched_slippers"):
            bits.append("門邊的拖鞋")
        if flags.get("s08_waited") or flags.get("s08_returned_early"):
            bits.append("牽繩那道鬆弧")
        elif flags.get("s08_forced_walk"):
            bits.append("被拉直的那條繩")
        if not bits:
            return None
        joined = "、".join(bits[:4])
        if gave_away:
            return joined + "。鑰匙那一聲，沒有誰接住。"
        return joined + "。牠還在。"
...
image gallery secret_sniff_wire = "gallery/secret-sniff-wire.png"
...
image bg bedroom_night = optional_background(
    "bg/bg-bedroom-night.png", "#211913"
)
...
image yuan home_sit = char_sprite(
...
transform char_center:
    ...
    ypos 0.80
    zoom 1.0
    xzoom sc_char("living_center")
    yzoom sc_char("living_center")
...
transform char_right:
    ...
    ypos 0.80
    zoom 1.0
    xzoom sc_char("living")
    yzoom sc_char("living")

# S07 臥室病床：沿床躺。zoom 0.18；xalign 靠右枕，勿蓋床頭燈；ypos 略上。
transform char_bedroom:
    xalign 0.78
    yanchor 1.0
    ypos 0.76
    zoom 1.0
    xzoom sc_char("bedroom")
    yzoom sc_char("bedroom")
...
transform char_convenience:
    ...
    yanchor 1.0
    ypos 0.80
    zoom s02_char("convenience")

transform char_convenience_right:
    xalign 0.74
    yanchor 1.0
    ypos 0.80
    zoom s02_char("convenience")

# 巷口／街：SCALE_S02 street。遇狗前與抱走同一把尺。
transform char_street:
    xalign 0.50
    yanchor 1.0
    ypos 0.80
    zoom s02_char("street")

transform char_street_carry:
    xalign 0.50
    yanchor 1.0
    ypos 0.80
    zoom s02_char("street")

# 辦公室：SCALE_S02 office。S02 開場站桌椅右側走道，勿疊上螢幕／椅背。
transform char_office:
    xalign 0.66
    yanchor 1.0
    ypos 0.80
    zoom s02_char("office")

# S02 抱狗進客廳：SCALE_S02 living（落地窗門框）。
transform char_living:
    xalign 0.74
    yanchor 1.0
    ypos 0.80
    zoom s02_char("living")

# 廚房 POV：SCALE kitchen（人對景；狗門檻深度例外）。
...
# S02 抱走後玄關：SCALE_S02 entrance；勿套日常 char_right_entrance。
transform char_entrance_carry:
    xalign 0.74
    yanchor 1.0
    ypos 0.80
    zoom s02_char("entrance")
...
# S07 指尖特寫：頭距已對齊 guard_door；另開近景 zoom（靠近看，不是 visH 全身尺）。
transform dog_bedroom_nose_cu:
    xalign 0.50
    yanchor 1.0
    ypos 0.80
    zoom 1.0
    xzoom sc_dog("bedroom_nose", True)
...
label reset_story_state:
    ...
    call maybe_show_howto
    return


label maybe_show_howto:
    ## 第一次進遊戲才出現；不教 Dist／Tone／Guard。
    if persistent.seen_howto:
        return
    call screen how_to_play
    $ persistent.seen_howto = True
    $ renpy.save_persistent()
    return
...
label section_01_fluorescent_over_moon:
    ...
    show yuan commute at char_convenience_right
    show clerk stand at char_convenience
    with dissolve
    ...
    show yuan commute at char_street
    with dissolve
    ...
    "世界變大：遠處的狗叫、樓上電視、自己的呼吸。關掉白噪音，外面那些聲音又回來了。"
    ...
    "她打開冰箱。"
    "低沉的嗡嗡從門縫裡漏出來......其實只是冰箱在運轉。"
    "門上的冷光落在腳邊。她盯著那塊空地，忽然想到：如果真把什麼帶回來，牠會待在哪裡？"
    "念頭才成形，她便把冰箱門關上。冷氣嗡聲停了一瞬，整間屋子更安靜......安靜一來，人就容易想太多。"
    thought "我連自己都常常忘記吃晚餐。"
    "通訊軟體最上面還停著家人一則未讀，日期是兩週前。她沒有點開。有時不讀，也是一種保持距離。"
    $ flags["ch2_seed_yuan_family_unread"] = True

    "手機搜尋框裡，她打了「路邊 小狗 沒力氣」。建議結果跳出脫水、失溫、通報電話。"
    "她沒有點進去。手指一個字一個字退回空白......沒搜尋過，心裡就輕一點。至少表面上。"
    "幾秒後，她又把通報電話抄在那筆舊備忘下面。只抄號碼，沒寫用途，也沒改過期日期。先準備好，再說要不要做。"
    ...
    if flags["peeked_backdoor"]:
        ...
        "予安睡著了。窗外有月亮，屋裡只有螢幕亮。"
        "那隻她遠遠看過一眼、卻沒走近的狗，是否還趴在後門陰影裡......她沒有答案。明天再說。"
    else:
        "意識往下沉。腳步聲在想像裡又走一遍：便利商店、微波的叮、那個「喔」、沒有走近的轉角。"
        "予安睡著了。窗外有月亮，屋裡只有螢幕亮。"
        "那隻她沒有去看的狗，是否還趴在後門陰影裡......她沒有答案。明天再說。"
...
## s02_conscience_return：趕走後回頭寫入；trust 可淨 0，Dist 維持 −1（回來≠沒趕過）。
...
label section_02_backdoor_glance:
    ...
    ## 耳機掛脖（勿戴上）；站桌椅右側走道，勿套客廳 char_center
    show yuan headphones_off at char_office
    with dissolve

    "週五傍晚。予安還是最後一批關螢幕的人。簡報交出去了，人卻還沒下班。"
    ...
    if flags["peeked_backdoor"]:
        "昨晚繞過去看過一眼。此刻腳步比較穩，像明知不該去，腳還是轉進去了。"
    else:
        "昨晚沒有繞過去。轉角忽然很近，近到來不及假裝路過。"
    ...
    "卸貨區比想像窄。機車棚陰影把地面切成深淺。垃圾桶邊，壓扁的紙箱翹著邊角。"
    ...
    # ▷ 風味（不改 trust）...... 記憶點加深：第一次對視可再看一眼
    menu:
        "再看牠一眼":
            $ flags["s02_looked_again"] = True
            pause 0.6
            "紙箱陰影裡，那雙眼睛又抬起來。這次是牠先眨眼。"
            "予安沒有伸手。便當袋的塑膠繩在指間發出很輕的聲。"

        "先把視線放到便當袋上":
            $ flags["s02_looked_again"] = False
            "她看向便當袋。狗沒有再對視，只把鼻子往紙箱裡縮半寸。"
    ...
    menu:
        "蹲下，側身等待（不伸手）":
            ...
            pause 0.7
            $ dog_sfx("soft")
            "很久，狗自己往前挪了半步。停在「碰得到氣味、碰不到手」的距離。尾巴不搖。"
            pause 0.5
            # ② 拿出飯盒 → 才更靠近
            "她掀開便當蓋，用筷子挑出一點白飯，放在掌心外的地上......離自己遠，離狗近。手收回來。"
            show dog sniff_bento at dog_backdoor_near
            with Dissolve(1.5)
            pause 0.7
            $ dog_sfx("soft")
            "飯香一落地，狗才再靠近。舌尖點了一下米粒，又點一下。每吞一口就抬眼看她。"
            "她的手指動了一下，又收回袖口。伸出去很容易；難的是，之後還站在原地。"
            thought "我可以什麼都不做。"
            pause 0.5
            # ③ 拿出水 → 退後一步
            "她把瓶蓋擰開，倒一點水推進去。塑膠蓋在水泥地上輕輕一響。"
            show dog s04_anxious at dog_backdoor_mid
            with Dissolve(0.8)
            pause 0.5
            $ dog_sfx("murmur")
            "狗往後退了一步。不是拒喝，是對手突然換動作的警戒。"
            ...
            pause 0.5
            $ dog_sfx("soft")
            "狗又往前。鼻尖先確認蓋緣，才低頭喝了兩口。"
            "喝完，牠停在「既不碰你、也不放你走」的距離。尾巴輕貼腿側。"
            pause 0.6
            "胸口某處鬆了一下。只是「至少今晚牠還喝得動」那種小小的鬆一口氣。"
        ...
        "拍腿趕開，轉身就走":
            ...
            show dog s04_anxious at dog_backdoor_far
            with Dissolve(0.8)
            "狗還在。眼睛亮著，耳朵卻仍貼平——回來的是人，不是剛才那聲嘖。"
            show yuan squat_side at char_backdoor_squat
            with Dissolve(0.5)
            "這次她蹲下來，手貼在自己腿側。距離沒有因為後悔自動重設。"
            $ play_bgm("tender", fade=2.5)
            show dog s04_anxious at dog_backdoor_far
            with Dissolve(1.0)
            pause 0.8
            "很久，鼻子才從紙箱裡伸出來一點。不後退，也不靠近。牠記得被趕過；也記得這個人又站回來。"
    ...
    if flags["dist_ok"]:
        ...
        show dog s04_anxious at dog_backdoor_far
        with Dissolve(0.8)
        "狗留在紙箱邊。鼻尖沒縮回陰影，身體也沒跟過來。"
        pause 0.6
        "予安站起來，往後退一步。狗沒跟。嘖聲還在；回頭也是真的。"
    else:
        "「動保明天才來。」她對狗說，也對自己說。"
        show dog s04_anxious at dog_backdoor_far
        with Dissolve(0.6)
        "狗往前一點，又縮回去。下巴壓低，盯著她的鞋尖，肩膀還緊。"
        pause 0.6
        "予安站起來，往後退一步。狗沒跟。"

    "店員從騎樓探頭：「小姐？要不要我幫你打給......」"
    ya "我知道了。我……先處理。"
    "語氣比較低、比較慢。不是對店員那種客套聲音。"
    ...
    "予安看著那隻狗。狗看著便當袋，又看她的臉，最後看向巷口。"
    ya "今晚不算數。"
    "聲音輕得像怕被巷子聽見。她把袖口往下拉，像給自己留退路。"
    ...
    if flags["dist_ok"]:
        ...
        pause 0.7
        "胸口某處鬆了一下。只是「至少今晚牠還喝得動」。"

    # ...... 離場鉤子 ......
    ya "我抱你一下。只是抱。很快。"
    "她事先預告，才伸手——跟狗說話一樣，先講一聲比較不嚇人。"
    ...
    scene bg street_night
    with Dissolve(1.5)
    show yuan carry_pup at char_street_carry
    with Dissolve(0.5)

    $ play_bgm("warm", fade=3.0)

    if flags.get("vet_first", False):
        "導航把她帶向夜間急診。狗的心臟隔著她的手臂跳得很快。她的心臟也是。"
    else:
    ...
    if flags.get("vet_first", False):
        ...
        "掛號單捏在掌心，比剛才那份便當收據還重。狗的鼻子埋進她外套褶裡，嗅消毒水、藥味，鼻尖埋得很深，像把整個人暫時藏進去。"
        hide yuan
        with Dissolve(0.5)
        "藥袋輕輕敲著手腕。診所的白光退了；接下來，只剩鐵門、門牌，與門縫底下那條細光。"
    else:
        "電梯門要關時，她用肩膀擋住。狗的鼻子埋進她外套褶裡，嗅加班、關東煮，鼻尖埋得很深，像把整個人暫時藏進去。"
        "門關上。樓層燈一格一格往上跳。"
        "她沒有數。第一次，她忘了數。"

        scene bg entrance_night
        with Dissolve(1.0)
        show yuan carry_pup at char_entrance_carry
        with Dissolve(0.4)
        ...
        scene bg living_night
        with Dissolve(1.0)
        show yuan carry_pup at char_living
        with Dissolve(0.4)
...
label section_03_gate_temp_border:
    ...
    menu:
        "開門蹲在兩步外，輕聲說「我還在」，再補水":
            ...
            ya "我還在。明天……再說。"
            "聲音放軟了，不是開會時那種尖銳。狗不懂句子，但聽得出：這個人沒在吼。鼻子伸向鞋尖，停在半公分外......想靠近，又還不敢。"
            $ dog_sfx("soft")
        ...
        "開門用腳把牠趕進角落，提高音量說「別擋路」":
            ...
            scene bg entrance_night
            with Dissolve(0.9)
            "她把大門關上。玄關燈還在嗡。安靜得過分......像剛把該聽的聲音關在門外。"
    ...
    menu:
        "輕輕引牠進屋，讓牠待在看得見的地方":
            ...
            if trust >= 2 or flags.get("s03_returned", False):
                ...
                with Dissolve(0.7)
                "跨過門檻時，牠全身還繃著。門一關，下巴才肯放下半公分......多靠近一點，牠都要再確認一次。"
            "她把門關好，燈只留這一盞。"
            scene bg living_night
            with Dissolve(1.2)
            show dog parallel at dog_mid
            with Dissolve(0.8)
            "外套鋪進客廳地板，離沙發兩步。狗轉了一圈，背朝門、臉朝她躺下。離沙發仍有兩步，卻已經在同一盞燈下面。眼睛還睜著，盯了她一會兒。"
    ...
    if flags.get("delayed_entry", False):
        ...
        scene bg living_night
        ## 客廳用 parallel；door_sleep 留給門外／門邊
        show dog parallel at dog_mid
        with Dissolve(0.8)
    ...
    if flags.get("delayed_entry", False):
        ...
        with Dissolve(0.6)
        "清晨。窗光先到地板。狗已經醒了，還在那兩步遠......怕還在，但人沒有消失。"

    ya "……早。"
    "狗不回答，只是抬著頭，視線越過地板，和她的眼睛對上。牠還在門裡。誰也沒先把眼睛轉開。"
...
## flags 寫入：s04_parallel／s04_forced_photo／bathroom_closed／s04_looked_again
## 畫面：予安室內坐書櫃前木椅（yuan home_sit／char_chair）；聲響 ear_perk → chin_hover → parallel → head_turn，選前回到 parallel
## 狗尾隨廚房用 dog wag＋分步換位；Dist 選 B＝street_tense＠far，選 C＝s04_low＠far
## ------------------------------------------------------------
...
label section_04_shared_quiet:
    ...
    "沒有新買的碗，沒有訓練課程，也沒有「從今天起我們是家人」這種台詞......像剛合租，誰也沒說好規矩。"
    ...
    $ s04_sound_far = not (trust >= 2 or flags.get("entered_home", False))
    if s04_sound_far:
        show dog ear_perk at dog_far_to_yuan
    else:
        show dog ear_perk at dog_chair_mid
    with Dissolve(0.45)
    "冷氣喀地一聲啟動。狗的耳朵豎起半秒又放下......牠不是在看她，是在確認屋子還會不會再出聲。"

    if s04_sound_far:
        show dog chin_hover at dog_far_to_yuan
    else:
        show dog chin_hover at dog_chair_mid
    with Dissolve(0.4)
    "電視忽然自己亮了一下，大概是遙控器被壓到，音量大得像整間辦公室被搬進了客廳。"
    "狗耳朵貼平，下巴離地只剩一公分......這屋子還會突然響，牠還沒習慣。"

    "予安把音量轉小，再小，小到幾乎只剩字幕在動。"
    if s04_sound_far:
        show dog parallel at dog_far_to_yuan
    else:
        show dog parallel at dog_chair_mid
    with Dissolve(0.7)
    pause 0.5
    "過了一會兒，狗的下巴才貼上地板......過好一會兒，耳朵才慢慢放下來。"

    "手機自動播到下一段短影片，笑聲忽然炸開，她慌忙按停，聲音斷在一半。"
    if s04_sound_far:
        show dog head_turn at dog_far_to_yuan
    else:
        show dog head_turn at dog_chair_mid
    with Dissolve(0.4)
    "狗沒有立刻趴回去，牠先看電視，再看手機，最後看她的手......像在找是誰在吵。"
    ...
    if s04_sound_far:
        ...
        show dog parallel at dog_chair_mid
    with Dissolve(0.5)
    "狗的耳朵朝她轉了一下......沒有靠近，也沒挪到更遠的地方。"
    thought "牠在看得見的地方就好。我也不用一直叫牠過來。"
    "她把原本要說的「過來」咬掉，改把雙腳往後收，留出一條從地板到門口的空路。這句話沒說出口。空路就留在那裡。"
    ...
    menu:
        "繼續各坐各的；牠在看得見的地方就好":
            ...
            with Dissolve(1.0)
            $ dog_sfx("sigh")
            "幾分鐘後，牠的耳朵鬆下來，下巴完整貼住地板......誰也沒動，就這樣待著。"
        ...
        "把牠硬抱上椅子，先合照一下":
            ...
            hide dog
            show dog street_tense at dog_far
            with Dissolve(0.7)
            pause 0.6
            $ dog_sfx("whimper")
            "予安按下快門，立刻把牠放回地板。照片裡她在笑，狗的眼睛卻只盯著逃跑的方向。"
        ...
        "把牠關進浴室；那裡比較好清理":
            ...
            show dog s04_low at dog_far
            with Dissolve(0.8)
            "狗沒有立刻出來。等牠終於走回客廳，選的是比原來更遠、也更靠近出口的位置......這道浴室的門，牠記住了。"

    "時間從上午滑到傍晚。予安回完兩封工作訊息，洗掉昨天的馬克杯，又坐回椅子......一天就這樣過去：回訊息、洗杯子、又坐回來。"
    if flags.get("s04_parallel", False):
        "狗抬眼看她幾次，沒有起身。她把這當成安靜成立，不是邀請......兩者常常長得很像。"
    else:
        "狗抬眼時先找出口。等她回到原位，牠才把下巴放下。"
    "窗光移過地板一塊又一塊。她起身倒水之前，那團蜷著的毛始終留在原位，像擺好的靠墊，動也不動。"

    "傍晚，她口渴，起身去廚房倒水。腳步一離開椅子，地板上那團也立刻跟著動了。"

    ## 尾隨：先定點，再分步往左下走向廚房門線，最後切廚房 POV
    hide yuan
    with Dissolve(0.45)
    show dog wag at dog_follow_start
    with Dissolve(0.7)
    pause 0.35
    show dog wag at dog_follow_mid
    with Dissolve(0.5)
    pause 0.35
    show dog wag at dog_follow_left
    with Dissolve(0.5)
    pause 0.35
    show dog wag at dog_follow_door
    with Dissolve(0.5)
    pause 0.5
    ...
    "予安在水槽邊站住，手裡的杯子涼涼的。"
    "她看著那雙停在門線上的前腳。牠沒有往出口退。人走進廚房，牠卻第一次跟到門線。"
    "她沒叫牠再走一步，也沒關廚房的門。有些邀請，用不著說出口。"

    # ▷ 風味（不改 trust）...... 記憶點加深：門檻可再看一眼
    menu:
        "再看門口那雙前腳一眼":
            $ flags["s04_looked_again"] = True
            pause 0.5
            "前腳還沒跨進來。牠抬眼看她，又看門檻，像在確認這條線今晚仍然有效。"
            "予安也沒有伸手。兩個人隔著一條沒畫出來的線，把安靜再延長了幾秒。"

        "倒完水就走回椅子":
            $ flags["s04_looked_again"] = False
            "她把視線收回杯子。有些畫面，看太久會變成要求。"
    ...
    if flags.get("s04_parallel", False):
        ...
        ## 客廳不用 dog-anxious（S02 留白尺，會比趴姿大約一倍）
        show dog chin_floor at dog_far
    with Dissolve(1.0)
    ...
    if flags.get("s04_parallel", False):
        show dog ear_perk at dog_chair_near
    else:
        show dog ear_perk at dog_far
    with Dissolve(0.35)
...
## flags 寫入：s05_soft_voice／s05_sharp_voice／s05_repaired／easter_egg_name
## 畫面：予安坐左（char_chair_left，面右）；狗在右側、不翻轉（面左看她）
## 趴姿可見高對齊 parallel；站／搭椅對齊胸寬（約趴的 1.2 倍，勿讓 chair_paw 突然變大）
## 狗姿：parallel／s05_anxious → head_up → sniff_wire（開會全景）→ chair_paw → chair_stuck → s05_ear_flat／s05_anxious
## 會後記憶點：sniff_wire＠dog_living_wire_cu（hide 予安）→ 回憶 sniff_wire
## ------------------------------------------------------------
...
label section_05_two_voices:
    ...
    "早上八點五十七分，予安把筆電墊高，確認鏡頭只拍得到書櫃和半面乾淨的牆......開鏡頭時，書櫃看起來特別整齊。"
    "耳機一戴上，她的肩膀也跟著坐直，像切換了另一個版本的自己。"

    show yuan headphones_sit at char_chair_left
    with dissolve
    ...
    if flags.get("s04_parallel", False) or trust >= 4:
        ...
        show dog s05_anxious at dog_far
        with Dissolve(0.8)
        "[dog_label]仍留在靠門的位置，不看螢幕，只用耳朵聽她在房間裡走動......耳朵比眼睛先知道她在不在。"
    ...
    if flags.get("s04_parallel", False) or trust >= 4:
        show dog head_up at dog_near
    else:
        show dog head_up at dog_far
    with Dissolve(0.5)
    ...
    menu:
        "先關麥克風、拿下耳機，再輕聲把牠抱下來":
            ...
            "予安先按下靜音。耳機拿下來的瞬間，房間忽然變大......冷氣聲、狗的呼吸、自己的心跳，全都聽得見。"
            show yuan headphones_off_sit at char_chair_left
            with Dissolve(0.5)
            ya "等一下喔。我不是在兇你。"
            "她的手臂從胸腹下方托住牠，慢慢放回地板，沒有順手一推了事。"
            show dog s05_ear_flat at dog_mid
            with Dissolve(0.5)
            "狗站在原地看她，耳朵先貼著，過了一會兒才鬆開一點......要過好一會兒，才肯再靠近一點。"
            pause 1.0
            show dog parallel at dog_near
            with Dissolve(0.8)
        ...
        "先急著把牠抱開，會後再蹲下來道歉":
            ...
            "她怕同事看見，手臂先把狗抱離椅邊......手抽得太急了，倒是一下子就抱開。"
            "狗落地後退了一步，耳朵壓低。予安已經打開麥克風，那句抱歉來不及說，只能心裡記著。"
            show dog s05_anxious at dog_mid
            with Dissolve(0.7)
            pause 0.5
            $ dog_sfx("murmur")
    ...
    if flags.get("s05_repaired", False):
        ...
        ya "剛剛……太大聲了。"
        "狗仍停在門邊，耳朵轉向她，沒有完全別開。門縫還留著。"
    else:
        "狗等到耳機不再傳出人的聲音，才慢慢抬起頭，像在確認沒事了。"

    $ play_bgm("tender", fade=2.2)

    ## 會後特寫：只留 bg＋放大層（image_scale.md §0.2）；解鎖回憶 sniff_wire
    ...
    $ unlock_secret_photo("sniff_wire")
    "予安把耳機放到桌上，將線收好。"
    ya "關了。"
    ...
    menu:
        ...
        "替牠換一個新名字":
            ...
            "予安把「[dog_label]」念得很輕。狗未必聽得懂名字的意思，耳朵還是朝她動了一下。"

    $ flags["easter_egg_name"] = check_easter_egg_name(dog_label)

    show dog wag at dog_mid
    with Dissolve(0.8)
    "名字落定的那幾秒，[dog_label]的尾巴輕輕搖了起來......弧度不大，卻是今天的第一次。"
...
## 讀取：flags["easter_egg_name"]（S05 寫入）→ 玄關隱藏一句
## ------------------------------------------------------------
...
label section_06_corridor_third_person:
    ...
    $ trust = max(0, min(12, trust))
    ## 走廊被看見：calm；選 A 擋下／進門後才轉 tender
    $ play_bgm("guard_corridor", fade=2.0)
    ...
    show neighbor idle at char_s06_neighbor
    ## 白天門邊空手站姿（勿 commute 便當袋）；先扶門，還沒決定擋不擋
    show yuan door_hold at char_s06_yuan
    show dog s05_stair_watch at dog_far_pair
    with dissolve

    "門外站著隔壁新搬來的鄰居。推車上疊著紙箱，最上面那箱歪了一角......看起來隨時會倒。"

    neighbor "妳有養狗喔？"
    "鄰居才剛開口，[dog_label]的耳朵已經先動了......比人更快聽出，這不是熟人。"
    show neighbor stand at char_s06_neighbor
    with Dissolve(0.45)
    neighbor "好可愛。妳新養的喔？"
    show dog s06_retreat at dog_far_pair
    with Dissolve(0.5)
    "予安還沒想好怎麼回答，[dog_label]倒是先退了：整隻往後，肩膀撞上門框。"
    neighbor "我摸一下，牠應該不會咬吧？"
    "鄰居往前一步，手已經伸出去了......像路上遇到可愛的小孩，還沒問過就先伸手摸頭。"
    show dog s06_watch_hand at dog_mid_pair
    with Dissolve(0.5)
    $ dog_sfx("murmur")

    if tone >= 1 or flags.get("s05_soft_voice", False):
        ## 選前只靠過去看手；完全躲腿留給選 A
        show dog s06_watch_hand at dog_behind_pair
        show yuan door_hold at char_s06_yuan
        with Dissolve(0.8)
        "狗的耳朵貼低，往她小腿邊靠了半步。鼻尖還隔著一點空氣，眼睛卻只盯著那隻手......像在看那隻手會不會停。"
    else:
        show dog s06_retreat at dog_far_pair
        with Dissolve(0.8)
    ...
    if tone >= 1 or flags.get("s05_soft_voice", False):
        ...
        show dog s06_watch_hand at dog_mid_pair
    with Dissolve(0.45)
    "推車輪子卡進磁磚縫，前後推了兩下......喀、喀。每響一次，[dog_label]的肩膀就往她腿後再縮一點。隔著褲管，能感覺到那一點輕輕在抖。"
    if not (tone >= 1 or flags.get("s05_soft_voice", False)):
        show dog s06_retreat at dog_far_pair
        with Dissolve(0.45)
    pause 0.7
    "鄰居不是要找麻煩。可這種時候最難開口......像有人已經幫你夾了菜，你很難說「我不要」。"
    thought "擋在中間就好，不必吵贏。"
    "牠看的不是那隻手有沒有伸過來，是那隻手會不會停......後門那次，也是這樣。"
    ...
    menu:
        "往前半步擋住，說「牠還在適應，先不要摸」":
            ...
            ya "我知道，謝謝妳。"
            "樓梯間安靜了兩秒，鄰居就把手收回去了。沒有吵，也沒有拉著要摸。"
            show neighbor withdraw at char_s06_neighbor
            show dog behind_legs at dog_behind_pair
            show yuan block at char_s06_yuan
            with Dissolve(0.5)
            pause 0.7
        ...
        "先讓場面過去，讓鄰居摸一下":
            ...
            pause 0.6
            neighbor "牠好乖喔。"
            "鄰居說牠乖。予安看著那條貼緊的尾巴，一時不知道該接什麼......牠只是沒地方可退。"
        ...
        "先把牠抱回屋裡，避免惹麻煩":
            ...
            with dissolve
            pause 0.5
            "門關上了。外頭沒人再伸手。可牠被抱進去的時候，沒看見她到底站在哪邊。"

    if flags.get("s06_sent_inside", False):
        ## 狗已在門內；予安回到門邊應鄰居，避免人消失還對答
        show yuan door_hold at char_s06_yuan
    show neighbor idle at char_s06_neighbor
    with Dissolve(0.4)
    neighbor "可是這棟可以養嗎？"
    ...
    if flags.get("s06_protected", False):
        "予安低頭，[dog_label]仍停在她腿後。這次沒有人再把手伸過來。"
    else:
        "予安看向門內，沒有立刻接話。"
    ...
    "予安點頭。走廊還是走廊，沒有人翻臉，也沒有人追問下去。"

    if flags.get("s06_protected", False):
        "電梯門關上。樓下還聽得到推車輪壓過磁磚縫，喀、喀，一下一下，比剛才那隻手還慢才聽不見。"
        "[dog_label]的耳朵跟著那聲音動，身體卻還貼在她小腿後。沒有人再伸手。走廊這才靜下來。"
    elif flags.get("s06_allowed_touch", False):
        "電梯門關上。鄰居的香水還留在門把那邊，一下子散不掉。"
        "[dog_label]在門邊偏過頭，去聞自己剛才被摸過的肩。予安沒解釋，只把鑰匙轉得比平常慢。"
    else:
        "電梯門關上。門內安靜得像剛才什麼都沒發生。"
        "[dog_label]對準門縫看了一眼，確認那隻手沒跟進來，下巴才放下來一點。"
    ...
    hide neighbor
    hide yuan
    ## 進屋後勿 commute（便當袋）。先空人，狗入場後再決定是否出予安。
    if flags.get("s06_sent_inside", False):
        show dog halfstep at dog_entrance_mid
        with Dissolve(0.5)
        "狗在鞋櫃旁等著。予安把門留開，自己退到兩步外，等牠要不要自己走出來。"
    else:
    ...
    ya "[dog_label]。"
    "聲音很輕。樓梯間剛關在外面，這個名字第一次在玄關被叫出來。"
    "[dog_label]的耳朵朝她動了一下，沒有走近。"
    $ _egg_line = easter_egg_reaction_line()
    if _egg_line:
        "[_egg_line]"
    ...
    if flags.get("s06_protected", False):
        ## 只看護衛：額頭輕碰＝剛才謝謝。B／C 即使信任夠也不給這一下。
        ## 特寫只留 bg＋放大層；禁客廳 dog_nudge、禁地板 entrance_mid。
        hide yuan
        show dog forehead_nudge at dog_entrance_nudge_cu
        with Dissolve(1.0)
        $ dog_sfx("soft")
        "[dog_label]往前半步，額頭很輕地碰了一下她的小腿......像輕輕點一下就好，碰完就退開。"
        $ unlock_secret_photo("forehead_nudge")
        ya "……不客氣。"
    ...
    "予安沒有急著說「沒事」。她先走到門邊確認鎖好，才回到牠看得見的位置坐下。"
    "過了一會兒，牠把抵在磁磚上的肩膀鬆開一點。"

    centered "{size=30}{color=#F7EFE4}今晚這樣站著，就夠了。{/color}{/size}"

    "她把這句話留給自己，沒打算說給誰聽。"
    "玄關燈還亮著。[dog_label]在鞋櫃旁繞了一圈，最後趴在能同時看見她、也看得見門的地方......兩邊都要看得到，才肯躺下來。"
    "予安沒有關燈。走廊還有一點聲響，先不要急著關掉。"
...
## flags 寫入：s07_reassured／s07_shut_out／s07_door_ajar／s07_fetched_slippers
## 演出四拍：進場（病床／門線）→ 關鍵（起身失敗＋耳鳴＋Tone）
##          → 反應（倒水／門線呼吸）→ 離場鉤子（辦公室看門邊照，不顯示予安）
## ------------------------------------------------------------
...
label section_07_sick_guard:
    ...
    "額頭燙，喉嚨乾，連吞口水都要先想一下。門外，有爪子刮過地板。"
    show dog s07_low at dog_bedroom_far
    with Dissolve(0.6)
    pause 0.35

    if trust >= 5 or flags.get("s06_protected", False):
        show dog halfstep at dog_bedroom_mid
        with Dissolve(0.55)
        pause 0.3
        show dog halfstep at dog_bedroom_near
        with Dissolve(0.7)
        "[dog_label]走到床邊。"
        show dog ear_perk at dog_bedroom_near
        with Dissolve(0.45)
        pause 0.35
        "鼻尖碰了一下垂落的被角。"
        show dog halfstep at dog_bedroom_mid
        ...
        show dog s07_low at dog_bedroom_mid
        with Dissolve(0.45)
        $ flags["s07_fetched_slippers"] = False
        "[dog_label]沒有進房，只在門口來回兩趟。"
        $ dog_sfx("bark")
    ...
    with Dissolve(0.35)
    $ dog_sfx("bark")
    "狗又叫了一聲。不太像在催她起床，比較像在問：裡面還有沒有人應。"

    "予安想撐起身體，手肘才離開床，視線就暗了一圈......頭一暈，只好重新躺下。棉被被汗黏在膝後。"
    "床邊的水杯是空的，手機在客廳充電。今晚兩樣都不在手邊。"
    show dog halfstep
    with Dissolve(0.4)
    pause 0.3
    "[dog_label]聽見床墊下沉，前腳往前半步。"
    show dog chin_hover
    with Dissolve(0.4)
    pause 0.35
    "又因她突然咳嗽停住。"
    ...
    $ dog_sfx("bark")
    pause 0.3
    "過了幾秒，[dog_label]短短叫了一聲。那聲音不大，卻把頭裡那陣尖聲壓下去。她這才又聽得見，門在哪一邊。"
    "她想應一聲，喉嚨只剩乾痛；想伸手，手臂還是抬不起來。這一次，她得先決定要怎麼回應那個聲音。"
    ...
    if flags.get("s07_shut_out", False):
        ...
        with Dissolve(0.9)
        hide yuan
        show dog halfstep at dog_bedroom_far
        with Dissolve(0.5)
        pause 0.3
        show dog guard_door at dog_bedroom_mid
        with Dissolve(0.6)
        "[dog_label]跟到房門口便停下。牠趴下來，頭朝客廳，耳朵卻留一隻向著她。"
        if flags.get("s04_parallel", False):
            "停的位置跟廚房門口差不多。牠還記得哪裡可以停、哪裡不要進去。"
        elif flags.get("bathroom_closed", False):
            "牠停在門外，比那天廚房門口更遠一點。門一關，牠就停得更遠。"
        elif flags.get("ch2_seed_water_bowl", False):
            "門檻這兒，牠在廚房門口見過一次。這次換了房間，停的方式還是一樣。"
        ya "謝啦。"
    ...
    hide yuan
    show dog nose_tip at dog_bedroom_nose_cu
    with Dissolve(0.8)
    pause 0.6
    "一個濕涼的鼻尖短短碰了一下，又退回去。就那一下，她又睡著了。"
    $ unlock_secret_photo("nose_touch")
    show dog guard_door at dog_bedroom_mid
    with Dissolve(0.6)
    "早上，她傳了一句請假。主管只回「好，先休息」。她打好的那一大段解釋，最後還是沒送出去。"
    if flags.get("ch2_seed_yuan_family_unread", False):
        "通知列往下滑時，家人那則未讀還停在原位。她沒有點開。"
    "予安忽然笑了一下。牠不會量體溫，也不會拿藥；可門邊那團影子整晚都沒離開......請假條上寫不出這種事。"
    "頭裡那陣尖銳的聲音淡了。房間裡只聽得見呼吸，一個在床上，一個在門邊。"

    scene bg office_night
    with Dissolve(1.5)
    ## 尾鉤不顯示予安；門邊照等旁白點進相簿後才淡入

    "退燒後第一個能進辦公室的傍晚，螢幕邊的夜燈已經亮了。"
    "她點進手機相簿。"
    show screen s07_phone_photo
    with Dissolve(1.2)
    pause 0.4
    $ unlock_secret_photo("door_sleep")
    "照片裡，[dog_label]睡在房門邊，一隻耳朵翻著。"
    if flags.get("s07_shut_out", False):
        "她把照片放大。門是開著的——那是她後來才打開的。照片裡看不出中間關了多久。"
    elif flags.get("s07_door_ajar", False):
        "門縫在照片裡只剩一條光。她盯著那條光，多停了兩秒。"
    else:
        "她沒有立刻滑走。三秒變成十秒。照片裡的耳朵，還是翻著。"
    "她沒有貼動態。螢幕還亮著，她多看了兩眼，才按暗。"
    hide screen s07_phone_photo
    with Dissolve(0.6)
    "這種照片，不用給別人看。"
    thought "那晚，門邊一直有牠。"
    "下班離開前，她繞進生活用品店，買了一條最普通的牽繩。"
    "她把牽繩放進提袋時，忽然想起房門口那雙還沒完全放下的耳朵。"
...
label section_08_corner_walk:
    ...
    if flags.get("s03_choice") == "shoo":
        ...
        "喝完，牠看了門一眼，才把下巴放下。像還在等這扇門會不會又只開一道縫。"
    "手腕被繩帶磨出一條淡紅色。她用拇指按了按，才發現自己從出門到現在一直握太緊。"
    "胸背帶的扣環在玄關解開時，[dog_label]全身抖了一下，把毛裡的灰一起甩開。"
...
label section_10_share_the_key:
    ...
    if flags.get("gave_away", False):
        ...
        $ _footprint = s10_footprint_line(gave_away=True)
        if _footprint:
            "[_footprint]"
        "她把鑰匙取下又掛回去，金屬碰牆的聲音，比平常清楚了不只一點。"
        thought "屋裡更靜，連鑰匙碰牆都變大聲。"
        "手機還是沒有訊息。她告訴自己，抵達新家總需要時間；告訴自己第二次，她才承認，自己其實在等一張不是自己拍的安全照片。"
        ...
        "店員問：「家裡養兩隻嗎？」她愣了半秒，搖頭。"
        ya "一隻。只是想多放一個。"
        "新的水碗被包進薄紙袋。提著走回家的路上，重量輕得幾乎沒感覺，卻像真的住下來了。"
        ...
        $ unlock_secret_photo("water_bowl")
...
label ending_ch1_back_to_back:
    ...
    ## 停電坐地板：清掉 sofa／通勤立繪，只留狗睡姿差
    hide yuan
    with Dissolve(0.3)

    "[dog_label]在黑暗裡抬起頭，先看窗外，再看地板上那圈光。牠沒有衝向門，也沒鑽進桌底，只把鼻子伸進光圈邊緣聞了一下，像在確認沒有危險。"
    ...
    "傍晚，鑰匙聲再次落在門外。[dog_label]仍趴在原位，只用尾巴輕輕碰了一下地板，算是回應。"
...
label ending_ch1_chosen_learning:
    ...
    ya "晚上見。"
    "狗看著她，沒有跟到門外。門線還在原位，牠自己停住了。"
    "傍晚，她開門時，[dog_label]先在原地確認了兩秒，才慢慢走到玄關一半。"
    "予安蹲下，卻沒有伸手。剩下那一半距離，不需要今晚就走完。"
    call ending_coda_finish("B", "結局 B｜選定但還在學", "還會回頭看；身體還沒跟上。")
    return
...
label ending_ch1_thin_ice:
    ...
    "電力恢復，冰箱啟動。狗又醒了一次。這回牠只是抬眼，沒有站起來，總算沒再縮回去。"
    "予安把新水碗往牆邊推近一點，再退回沙發。每個動作都讓牠看得清清楚楚，不趁轉身縮短距離耍花招。"
    "那一夜沒有靠得更近。只有兩個睡得都不深的生命，隔著客廳，慢慢確認對方沒趁黑暗改位置。"
...
label ending_aftercare:
    ...
    if flags.get("ch1_ending") == "chosen_learning":
        ...
        if secret_photo_unlocked("lap_sleep"):
            "有一段後來才看得見的畫面......主選單裡，照片會自己多出來。"
            "若想再讀，主選單也多了幾篇只在走完之後才打開的文字。"
    elif flags.get("ch1_ending") == "handed_over":
        if flags.get("landmark_chose_reason_over_bond", False):
            "在關係最好的時候把牽繩交出去......那股酸，比任何一句解釋都難說出口。"
        else:
