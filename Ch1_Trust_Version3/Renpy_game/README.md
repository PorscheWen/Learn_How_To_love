...
微調章節對白、旁白、心裡話與選項文字（寫回 `game/script.rpy`）：

```powershell
.\開啟對話修改器.bat
```

瀏覽器會開啟 `http://127.0.0.1:8787/`。左側選 S01～S10／結局，右側編輯後按「儲存變更」。只改字串，不動 `$ flags`／跳轉邏輯。Ctrl+C 結束伺服器。
...
- **Section 02｜後門那一瞥：** 她終於轉進後門。紙箱邊那一瞥，讓腳步自己慢了下來。
...
- **Section 06｜樓梯間的第三者：** 鄰居伸手要摸；她得決定，自己要站在哪裡。
- **Section 07｜她倒下的那天：** 予安病得起不了身，小7不懂怎麼照顧人，只知道守在門口，試著等她回應。
- **Section 08｜走到轉角就好：** 第一次出門只為抵達巷口；世界太吵時，予安得決定要拉著牠，還是一起停下。
- **Section 09｜差點交給別人：** 同事真誠提出接手；交繩前，她得承認誰已經選過誰。
- **Section 10｜把鑰匙分給心跳：** 鑰匙與牽繩掛在同一面牆上；夜深後，牠願不願意靠近，會把答案留在地板上。
...
│  ├─ dog-s04-anxious.png（S02 後門）
│  ├─ dog-anxious.png（S06～低信任備援）
│  ├─ dog-halfstep.png
│  ├─ dog-stair-watch.png
│  ├─ dog-door-sleep.png
│  ├─ dog-parallel.png
│  ├─ dog-ear-perk.png／dog-chin-hover.png／dog-head-turn.png（S04 聲響）
│  ├─ dog-head-up.png／dog-chair-paw.png／dog-chair-stuck.png（S05 早會）
│  ├─ dog-chin-floor.png
...
│  ├─ dog-door-edge.png
│  ├─ wag/
│  │  └─ dog-wag-01.png … dog-wag-05.png（S04 尾隨／S05 搖尾巴）
